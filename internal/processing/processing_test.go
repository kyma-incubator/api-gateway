...
var _ = Describe("Factory", func() {
	Describe("CalculateRequiredState", func() {
		Context("APIRule", func() {
			...
			It("should produce VS and ARs for the same paths", func() {
				noop := []*rulev1alpha1.Authenticator{
					{
						Handler: &rulev1alpha1.Handler{
							Name: "noop",
				...
				jwtRule := getRuleFor(headersAPIPath, []string{"POST"}, testMutators, jwt)
				rules := []gatewayv1alpha1.Rule{noopRule, jwtRule}
				...
				//Verify rules
				Expect(len(accessRules[expectedNoopRuleMatchURL])).To(Equal(2))
				...
				jwtAccessRule := accessRules[expectedNoopRuleMatchURL][1]
				...
				Expect(len(jwtAccessRule.Spec.Match.Methods)).To(Equal(len(apiMethods)))
				Expect(jwtAccessRule.Spec.Match.Methods).To(Equal([]string{"POST"}))
				Expect(jwtAccessRule.Spec.Match.URL).To(Equal(expectedNoopRuleMatchURL))
